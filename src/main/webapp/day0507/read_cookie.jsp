...
if(cookies != null){
//out.println(cookies.length); //JSESSIONID가 하나 존재
...
for(int i=0; i<cookies.length; i++){
	...
	if(!"JSESSIONID".equals(tempCookie.getName())){
		cookieFlag=true;//JSESSIONID가 아닌 쿠키가 있니?
		//쿠키의 값을 얻기		
		...
		if("saveFlag".equals(tempCookie.getName())){
			saveFlag="on".equals(tempCookie.getValue());
...
%>
<%
//if(!cookieFlag){//쿠키가 존재하지 않으면 이름과 나이를 입력할 수 있는 
	//HTML Form Control을 제공
%>
<form action="add_cookie.jsp" method="post" id="frm">
<label for="name">이름:</label>
<input type="text" name="name" id="name" value="<%= saveFlag?name:""%>"/>
<input type="checkbox" name="saveFlag"<%=saveFlag? "checked='checked'":"" %> />이름저장<br>
