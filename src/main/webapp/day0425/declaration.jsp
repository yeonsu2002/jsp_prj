...
<%=msg() %><br>
<%
	String[] nameArr={"강태일", "이장훈", "양준수" ,"이호빈" ,"주현석"};
%>
...
<td><%= nameArr[i] %></td>
<td>
<%= MyUtil.createRadio("score", i, 10, 5) %>
<%-- <%for(int score=0; score<11; score++){ %>
<input type="radio" name="score<%=i %>" value="<%=score %> " <%= score == 0 ? "checked='checked'" : "" %> /><%=score %>
<% } %> --%>
</td>
</tr>
<%} %>
